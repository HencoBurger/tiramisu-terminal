...
export function useAgent() {
  async function agentStart(tabId: string, provider: string, model: string, workerModel: string, workDir: string, prompt: string, images: string[] = []) {
    await AgentStart(tabId, provider, model, workerModel, workDir, prompt, images)
  }
  async function agentSend(tabId: string, provider: string, model: string, workerModel: string, workDir: string, prompt: string, images: string[] = []) {
    await AgentSend(tabId, provider, model, workerModel, workDir, prompt, images)
  }
